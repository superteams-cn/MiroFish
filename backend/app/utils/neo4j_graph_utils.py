...
class Neo4jGraphClient:
    ...
    def build_indices_and_constraints(self):
        with self.driver.session() as session:
            session.run("CREATE CONSTRAINT mirofish_entity_uuid_unique IF NOT EXISTS FOR (n:Entity) REQUIRE n.uuid IS UNIQUE")
    ...
    def write_graph(
        ...
        with self.driver.session() as session:
            for node in nodes:
                labels = [label for label in node.get("labels", []) if label and label not in ("Entity", "Node")]
...
def fetch_all_nodes(
    client: Optional[Neo4jGraphClient],
    group_id: str,
...
def delete_group(client: Optional[Neo4jGraphClient], group_id: str) -> None:
    client = client or get_neo4j_graph_client()
    with client.driver.session() as session:
        session.run("MATCH (s:Entity)-[r:RELATES_TO]->(t:Entity) WHERE r.group_id = $gid DELETE r", {"gid": group_id})
