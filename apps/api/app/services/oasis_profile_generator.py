...
class OasisProfileGenerator:
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> dict[str, Any]:
        ...
        if not self._graph_client or not self.graph_id:
            logger.debug("跳过图谱检索：client 或 graph_id 未设置")
            return results
        ...
        try:
            ...
            related_edges = [
                ...
                or edge.get("target_node_uuid") == entity_uuid
            ]
            ...
            logger.info(
                f"图谱检索完成: {entity_name}, 获取 {len(facts)} 条事实, {len(summaries)} 个相关节点"
            )
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        ...
        return normalized in self.INDIVIDUAL_ENTITY_TYPES or any(
            token in entity_type
            for token in self.INDIVIDUAL_ENTITY_TYPES
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """判断是否是群体/机构类型实体"""
        normalized = entity_type.lower()
        return normalized in self.GROUP_ENTITY_TYPES or any(
            token in entity_type
            for token in self.GROUP_ENTITY_TYPES
            if any("\u4e00" <= ch <= "\u9fff" for ch in token)
        )
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                # 通用解析/截断/修复由 LLMClient 承载；取原文以便失败时做领域兜底（正则抽 bio/persona）
                ...
                try:
                    ...
                    logger.warning(f"JSON解析失败 (attempt {attempt + 1})，尝试领域兜底提取")
                    result = self._salvage_profile_json(
