...
- 请先根据模拟场景与参与群体，推断其所在时区、文化与作息习惯，再据此设定各时段活跃度
- 用「相对时段」而非固定钟点来理解(具体钟点随时区/文化平移)：
  - 深夜时段：几乎无人活动（活跃度系数约0.05）
  - 清晨时段：逐渐活跃（约0.4）
  - 工作/白天时段：中等活跃（约0.7）
  - 晚间高峰时段：最活跃（约1.5）
  - 深夜前回落（约0.5）
- 一般规律：深夜低、清晨渐增、工作时段中等、晚间高峰；但**务必按目标群体所在地域调整钟点边界**
- **重要**：以上系数仅供参考，请根据事件性质、参与群体与地域调整具体时段
  - 例如：学生夜间更活跃；媒体常全天活跃；官方机构多在当地工作时间
...
## 任务
为每个实体生成活动配置，注意：
- **时间符合目标用户群体作息**：钟点请按目标群体所在时区/文化推断,以下时段为「相对参考」,需据此平移
- **官方机构**（如 University/GovernmentAgency）：活跃度低(0.1-0.3)，仅当地工作时段活动，响应慢(60-240分钟)，影响力高(2.5-3.0)
- **媒体**（如 MediaOutlet）：活跃度中(0.4-0.6)，近乎全天活动，响应快(5-30分钟)，影响力高(2.0-2.5)
- **个人**（如 Student/Person/Alumni）：活跃度高(0.6-0.9)，以工作/学习外的闲暇时段为主，响应快(1-15分钟)，影响力低(0.8-1.2)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<活跃小时列表(0-23)，按目标群体所在时区/作息推断>],
