...
你的 __ENTITY_TOTAL__ 个实体类型必须包含以下层次：

A. **兜底类型（必须包含，放在列表最后2个）**：
   - 中文输出时使用 `个人`；英文输出时使用 `Person`。表示任何自然人个体的兜底类型。
   - 中文输出时使用 `组织`；英文输出时使用 `Organization`。表示任何组织机构的兜底类型。

B. **具体类型（__ENTITY_SPECIFIC__个，根据文本内容设计）**：
...
3. 前{max(1, entity_total - 2)}个是根据文本内容设计的具体类型
    ...
    def _validate_and_process(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        for entity in result["entity_types"]:
            if "name" in entity:
                ...
                entity["name"] = str(original_name).strip()
                entity_name_map[original_name] = entity["name"]
            entity.pop("display" + "_name", None)
        ...
        for edge in result["edge_types"]:
            if "name" in edge:
                ...
                edge["name"] = str(original_name).strip()
            edge.pop("display" + "_name", None)
